...
object Problem11 {
  ...
  case class Block(cells: List[Int]) {
    ...
    def highestProductInLine(num: Int)(line: List[Int]): Int = {
      val (slice, remain) = line.splitAt(num)
      remain.foldLeft((slice, slice.product)) { (z, i) =>
        val newSlice = z._1.tail.:+(i)
        (newSlice, z._2.max(newSlice.product))
      }
    }._2

    def highestProduct(num: Int): Int = {
      (rows ++ columns ++ diagDown ++ diagDown).filter(_.length >= num).map(highestProductInLine(num)).max
    }

  }

  val problemNumbers: List[Int] =
    """
    ...
    """.stripMargin.replace("\n", " ").split(" ").filterNot(_ == "").map(_.toInt).toList

  def main(args: Array[String]): Unit = {
    println(s"Answer: ${Block(problemNumbers).highestProduct(4)}")
  }
